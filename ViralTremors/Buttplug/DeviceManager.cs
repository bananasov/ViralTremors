...
public class DeviceManager
{
    ...
    public DeviceManager(string clientName)
    {
        ...
        ViralTremors.Logger.LogInfo("BP client created for " + clientName);
    ...
    public async void ConnectDevices()
    {
        ...
        try
        {
            ViralTremors.Logger.LogInfo($"Attempting to connect to Intiface server at {Config.ServerUri.Value}");
            await ButtplugClient.ConnectAsync(new ButtplugWebsocketConnector(new Uri(Config.ServerUri.Value)));
            ViralTremors.Logger.LogInfo("Connection successful. Beginning scan for devices");
            await ButtplugClient.StartScanningAsync();
        }
        catch (ButtplugException exception)
        {
            ViralTremors.Logger.LogError(
            ...
            ViralTremors.Logger.LogDebug($"ButtplugIO error occured while connecting devices: {exception}");
        }
    }
    ...
    private void HandleDeviceAdded(object sender, DeviceAddedEventArgs args)
    {
        ...
        {
            ViralTremors.Logger.LogInfo($"{args.Device.Name} was detected but ignored due to it not being vibratable.");
            return;
        }

        ViralTremors.Logger.LogInfo($"{args.Device.Name} connected to client {ButtplugClient.Name}");
    ...
    private void HandleDeviceRemoved(object sender, DeviceRemovedEventArgs args)
    {
        ...
        ViralTremors.Logger.LogInfo($"{args.Device.Name} disconnected from client {ButtplugClient.Name}");
